...
def make():
    ...
    for p in pages:
        ...
        rows.append([college_name(p),p,page_type(p),status,modified,"Review","Pass" if wc>=1200 else "Review","Pass" if s else "Review","Pass" if s else "Review","Pass" if s else "Review","Review","Review","Review"," ".join(notes),run(["git","log","-1","--format=%H","--",p]),score])
    ...
    stats=[("Pages tracked",len(rows)),("Audited",sum(r[3]=="AUDITED" for r in rows)),("In queue",sum(r[3]=="IN QUEUE" for r in rows)),("Modified / verified",sum("MODIFIED" in r[4] for r in rows))]
