...
This agent is deliberately self-contained. It does not import, read or execute
AGENTS.md, .github/agents, or .github/skills as instruction sources.
"""
...
def gemini_json(prompt,max_tokens=30000):
    ...
    r=session.post(url,params={"key":API_KEY},json=payload,timeout=180); r.raise_for_status()
...
FACT RULE: Use ONLY the supplied official-source material for factual claims. Never invent or infer fees,
dates, seats, salaries, cutoffs, eligibility, rankings, recruiters, programme names or statistics. If a fact
is unavailable, state that the official source reviewed does not publish/confirm it. Reddit/Quora may be used
ONLY for cutoff information when an official cutoff is absent, and only when evidence is supplied.

DESIGN RULE: Approved IIM Ahmedabad and SIBM Pune references define architecture, content depth, hierarchy
and responsive presentation. Use the shared college-page.css implementation. Do not copy their wording or
facts. Use a concise hero, quick facts, desktop On This Page sidebar, mobile On This Page control,
cards/tables/answer boxes where useful, FAQs and official sources. Answer first, student-centric, simple English.

SCOPE: Create one overview page, one placement page, and up to three high-intent MBA/PGDM/Executive MBA or
closely related management programme pages supported by official sources. Do not create unrelated UG, PhD or
certificate pages.
...
OG metadata/JSON-LD; FAQs; official source links; internal links only to known existing/package files; no
markdown, placeholders, lorem ipsum or unsupported numbers.
...
def priority_rows(rows):
    ...
    with PRIORITY.open(encoding="utf-8-sig",newline="") as f:
        for p in csv.DictReader(f):
            ...
            if r:
                r["college_name"]=p["college_name"]; r["official_website"]=p["official_website"]; out.append(r)
...
def update_xlsx(rows):
    ...
    if path.exists():
        wb=load_workbook(path); ws=wb.active; ws.delete_rows(1,ws.max_row)
    else:
...
def main():
    ...
    for r in eligible:
        ...
        if package_score>PUBLISH_THRESHOLD and not critical_all:
            ...
            git("add",*[fn for fn,_ in safe],"data/college-content-master.csv","data/college-content-master.xlsx",f"quality-audit-{slugify(college)}.json")
            ...
            git("add","data/college-content-master.csv","data/college-content-master.xlsx",f"quality-audit-{slugify(college)}.json",*[fn for fn,_ in safe]); git("commit","-m",f"Hold {college} pages for QA revision"); git("push","origin","main"); existing.update(package); generated_count+=1
