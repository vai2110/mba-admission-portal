#!/usr/bin/env python3
"""Independent MBA college content production agent v4."""
...
def revision_prompt(college,rank,official_url,research,types,failures,previous):
    return f'''Revise the following independently generated MBA college pages for {college}. Use ONLY the supplied official-source material. Fix these audit failures: {failures}. Preserve verified facts, remove unsupported claims, and improve structure, metadata, FAQs, source links and responsive architecture. Generate ONLY these page types: {", ".join(sorted(types))}. Return the same JSON format with complete HTML5 pages and source_urls. PREVIOUS OUTPUT: {json.dumps(previous)[:120000]} OFFICIAL SOURCE MATERIAL: {research[:160000]}'''
...
def main():
    ...
    for r in [x for x in priority_rows(rows) if any(not str(x.get(k,"")).strip().lower()=="done" for k in ("overview_status","placement_status","popular_course_status"))]:
        ...
        if package_score<=PUBLISH_THRESHOLD or critical_all:
            revision=gemini_json(revision_prompt(college,r["rank"],official_url,research,types,critical_all+notes,pages));revised=[p for p in revision.get("pages",[]) if p.get("type") in types and p.get("html")]
            if revised:
                safe=[(Path(p.get("filename","")).name,p) for p in revised if Path(p.get("filename","")).name];scores=[];critical_all=[];notes=[];package={fn for fn,_ in safe}
                for fn,p in safe:
                    sc,cr,no=audit_html(p["html"],p.get("source_urls",[]),official_url,existing|package);scores.append(sc);critical_all += [f"{fn}: {x}" for x in cr];notes += [f"{fn}: {x}" for x in no];(ROOT/fn).write_text(p["html"],encoding="utf-8")
                package_score=round(sum(scores)/len(scores))
