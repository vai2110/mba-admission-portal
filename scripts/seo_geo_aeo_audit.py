...
def attr_value(text, tag, attr, value):
    pattern = rf'(<{tag}\b[^>]*\b{attr}=["\']{re.escape(value)}["\'][^>]*\bcontent=["\'])(.*?)(["\'])'
...
def extract_description(text, title):
    m = re.search(r'<meta\b[^>]*\bname=["\']description["\'][^>]*\bcontent=["\'](.*?)["\'][^>]*>', text, flags=re.I | re.S)
    if m:
...
def add_schema(text, page_url, title, description, location, faq_items):
    # Remove only schema generated by this script, preserving all existing scripts.
