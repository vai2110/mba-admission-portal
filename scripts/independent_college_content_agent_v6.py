#!/usr/bin/env python3
"""Independent MBA college content production agent v6."""
...
def missing_types(college,tracker,forced=None):
    ...
    def exists(v):
        s=(v or "").strip().lower()
        return s not in ("","pending","not started","not started - pending","n/a","not applicable")
    m=set()
...
def generation_prompt(college,rank,url,research,types):
    wanted=", ".join(sorted(types))
    return f"""You are an independent MBA admissions content generator. College: {college}. NIRF 2025 rank: {rank}. Official domain: {url}. Generate ONLY these missing page types: {wanted}.\n\nUse ONLY supplied official-source material for factual claims. Never invent or infer fees, dates, seats, salaries, cutoffs, eligibility, rankings, recruiters, programme names or statistics. If unavailable, say so. Reddit/Quora are permitted ONLY for cutoff information when official cutoff data is absent and evidence is supplied.\n\nUse the approved IIM Ahmedabad and SIBM Pune reference architecture, hierarchy, content depth and responsive presentation. Use shared college-page.css. Do not copy benchmark wording or facts. Student-centric, simple English, answer-first.\n\nReturn JSON ONLY: {{\"pages\":[{{\"filename\":\"slug.html\",\"type\":\"overview|placement|programme\",\"title\":\"...\",\"html\":\"<complete HTML5 document>\",\"source_urls\":[\"official URL\"]}}]}}\n\nHTML must contain one H1, logical H2/H3, title, meta description, canonical, OG metadata, JSON-LD, FAQs and official source links. Internal links may target only known existing/package files. No markdown, placeholders, lorem ipsum or unsupported numbers.\n\nOFFICIAL SOURCE MATERIAL:\n{research[:180000]}"""
...
def main():
    ...
    for r in candidates:
        ...
        data=gemini_json(generation_prompt(college,r["rank"],url,research,types));pages=data.get("pages",[]);safe=[]
        for p in pages:
