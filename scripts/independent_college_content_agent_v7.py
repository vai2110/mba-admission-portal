#!/usr/bin/env python3
"""Independent MBA college content production agent v7."""
...
def discover_official_url(college,supplied_url):
    ...
    try:
        r=requests.get("https://www.google.com/search",params={"q":f'"{college}" official website'},timeout=TIMEOUT,headers={"User-Agent":"Mozilla/5.0"});soup=BeautifulSoup(r.text,"html.parser");bad=("wikipedia.org","collegedunia.com","shiksha.com","careers360.com","collegedekho.com","getmyuni.com","facebook.com","linkedin.com")
...
def response_pages(data):
    ...
    if isinstance(data,dict):return data.get("pages",[])
    return []
def read_master():
...
def tracker_for(college,tracker):
    ...
    for r in tracker:
        ...
        if tk & rk:return r
        if any(len(a)>=4 and any(a in b or b in a for b in rk) for a in tk):return r
    return None
def missing_types(college,tracker,forced=None):
    ...
    def exists(v):return (v or "").strip().lower() not in ("","pending","not started","not started - pending","n/a","not applicable")
    m=set()
    if not exists(r.get("Overview Page")):m.add("overview")
    if not exists(r.get("Placement Page")):m.add("placement")
    courses=[r.get(k,"") for k in ("Course Page 1","Course Page 2","Course Page 3")]
    if courses and not any(exists(v) for v in courses) and not all((v or "").strip().lower()=="not applicable" for v in courses):m.add("programme")
    return m
def generation_prompt(college,rank,url,research,types):
    return f"""You are an independent MBA admissions content generator. College: {college}. NIRF 2025 rank: {rank}. Official domain: {url}. Generate ONLY these missing page types: {', '.join(sorted(types))}. Use ONLY supplied official-source material for factual claims. Never invent or infer fees, dates, seats, salaries, cutoffs, eligibility, rankings, recruiters, programme names or statistics. If unavailable, say so. Reddit/Quora are permitted ONLY for cutoff information when official cutoff data is absent and evidence is supplied. Use the approved IIM Ahmedabad and SIBM Pune reference architecture, hierarchy, content depth and responsive presentation. Use shared college-page.css. Do not copy benchmark wording or facts. Student-centric, simple English, answer-first. Return JSON ONLY: {{\"pages\":[{{\"filename\":\"slug.html\",\"type\":\"overview|placement|programme\",\"title\":\"...\",\"html\":\"<complete HTML5 document>\",\"source_urls\":[\"official URL\"]}}]}}. HTML must contain one H1, logical H2/H3, title, meta description, canonical, OG metadata, JSON-LD, FAQs and official source links. Internal links may target only known existing/package files. No markdown, placeholders, lorem ipsum or unsupported numbers.\n\nOFFICIAL SOURCE MATERIAL:\n{research[:180000]}"""
def revision_prompt(college,rank,url,research,types,failures,previous):
    return f"""Revise these MBA admissions pages for {college}. Use ONLY supplied official sources. Fix: {failures}. Preserve verified facts, remove unsupported claims, improve metadata, structure, FAQs, source links and responsive architecture. Generate ONLY: {', '.join(sorted(types))}. Return JSON with complete HTML5 pages and source_urls. PREVIOUS: {json.dumps(previous)[:120000]}\nSOURCES:\n{research[:160000]}"""
...
def main():
    ...
    for r in priority_rows(rows):
        ...
        if not url:
            r["research_status"]="Blocked - official site not discovered";r["qa_status"]="Failed";r["quality_score"]="0";write_master(rows);update_xlsx(rows);generated+=1;continue
        ...
        data=gemini_json(generation_prompt(college,r["rank"],url,research,types));pages=response_pages(data);safe=[]
        for p in pages:
        ...
        if not safe:
            r["qa_status"]="Failed - generator returned no missing page";r["quality_score"]="0";write_master(rows);update_xlsx(rows);generated+=1;continue
        ...
        if package_score<=PUBLISH_THRESHOLD or critical_all:
            revision=gemini_json(revision_prompt(college,r["rank"],url,research,types,critical_all+notes,pages));rev=[p for p in response_pages(revision) if p.get("type") in types and p.get("html")]
            if rev:
        ...
        if package_score>PUBLISH_THRESHOLD and not critical_all:
            ...
            for fn,_ in safe:
                live=f"https://vai2110.github.io/mba-admission-portal/{fn}";ok=False
                for _ in range(8):
                    try:
                        if session.get(live,timeout=20).status_code==200:ok=True;break
                    except requests.RequestException:pass
                    time.sleep(10)
                urls.append(ok)
