#!/usr/bin/env python3
"""Independent MBA college content production agent v3."""
...
MASTER=ROOT/"data"/"college-content-master.csv";PRIORITY=ROOT/"data"/"college-content-priority.csv";TRACKER=ROOT/"college-production-tracker.csv"
...
def official_crawl(start_url,limit=28):
    ...
    return pages
def research_block(official_url):
...
def tracker_for(college,tracker):
    def keys(s):
        ...
        for a,b in [("indian institute of management","iim"),("indian institute of technology","iit"),("national institute of technology","nit"),("institute of management technology","imt"),("management development institute","mdi"),("symbiosis institute of business management","sibm"),("t. a. pai management institute","tapmi"),("great lakes institute of management","great lakes"),("chandigarh university","cu")]:
            if a in raw:variants.add(norm(raw.replace(a,b)))
        return variants
    tk=keys(college)
    for r in tracker:
        if tk & keys(r.get("College Name","")):return r
    return None
def missing_types(college,tracker):
    ...
    missing=set()
    if not created(r.get("Overview Page")):missing.add("overview")
    if not created(r.get("Placement Page")):missing.add("placement")
    courses=[r.get(k,"") for k in ("Course Page 1","Course Page 2","Course Page 3")]
    if courses and not any(created(v) for v in courses) and not all((v or "").strip().lower()=="not applicable" for v in courses):missing.add("programme")
    return missing
def generation_prompt(college,rank,official_url,research,types):
    return f'''You are an independent MBA admissions content generator.\nCOLLEGE: {college}\nNIRF 2025 RANK: {rank}\nOFFICIAL DOMAIN: {official_url}\nGENERATE ONLY: {", ".join(sorted(types))}\n\nUse ONLY supplied official-source material for factual claims. Never invent or infer fees, dates, seats, salaries, cutoffs, eligibility, rankings, recruiters, programme names or statistics. If unavailable, state that the official source reviewed does not publish/confirm it. Reddit/Quora are permitted ONLY for cutoff information when official cutoff data is absent and evidence is supplied.\n\nApproved IIM Ahmedabad and SIBM Pune references define architecture, hierarchy, content depth and responsive presentation. Use shared college-page.css. Do not copy their wording or facts. Use hero, quick facts, desktop On This Page sidebar, mobile On This Page control, cards/tables/answer boxes, FAQs and official sources. Answer first, simple English.\n\nCreate only the requested missing page types. MBA, PGDM, Executive MBA and closely related management programmes only.\n\nReturn JSON ONLY: {{"pages":[{{"filename":"slug.html","type":"overview|placement|programme","title":"...","html":"<complete HTML5 document>","source_urls":["official URL"]}}]}}\nHTML must include one H1, logical H2/H3, title, meta description, canonical, OG metadata, JSON-LD, FAQs and official source links. Internal links may target only known existing/package files. No markdown, placeholders, lorem ipsum or unsupported numbers.\n\nOFFICIAL SOURCE MATERIAL:\n{research[:180000]}'''
def audit_html(html,source_urls,official_url,all_repo_files):
    ...
    official_domain=urlparse(official_url).netloc.lower().replace("www.","")
    if any(urlparse(u).netloc.lower().replace("www.","")!=official_domain for u in source_urls):critical.append("non-official source URL")
...
def main():
    rows=read_master();tracker=read_tracker();existing={p.name for p in ROOT.glob("*.html")};generated_count=0
    candidates=priority_rows(rows)
    eligible=[r for r in candidates if any(not str(r.get(k,"")).strip().lower()=="done" for k in ("overview_status","placement_status","popular_course_status"))]
    for r in eligible:
        if generated_count>=BATCH_SIZE:break
        college=r["college_name"].strip();types=missing_types(college,tracker)
        if not types:
        ...
        r["official_website"]=official_url;research,source_pages=research_block(official_url);r["research_status"]="Research Complete";write_master(rows);update_xlsx(rows)
        data=gemini_json(generation_prompt(college,r["rank"],official_url,research,types));pages=data.get("pages",[]);safe=[]
        for p in pages:
        ...
        for fn,p in safe:
            sc,cr,no=audit_html(p["html"],p.get("source_urls",[]),official_url,existing|package);scores.append(sc);critical_all += [f"{fn}: {x}" for x in cr];notes += [f"{fn}: {x}" for x in no];(ROOT/fn).write_text(p["html"],encoding="utf-8")
        ...
        if package_score>PUBLISH_THRESHOLD and not critical_all:
            r["deployment_status"]="Eligible - Auto Publish";write_master(rows);update_xlsx(rows);git("config","user.email","41898282+github-actions[bot]@users.noreply.github.com");git("config","user.name","github-actions[bot]");git("add",*[fn for fn,_ in safe],"data/college-content-master.csv","data/college-content-master.xlsx",f"quality-audit-{slugify(college)}.json");git("commit","-m",f"Publish independently generated pages for {college}");git("push","origin","main");r["deployment_status"]="Pushed - Awaiting Pages";write_master(rows);update_xlsx(rows);urls=[]
            for fn,_ in safe:
            ...
            r["deployment_status"]="Blocked - QA <= 70 or critical failure";write_master(rows);update_xlsx(rows);git("config","user.email","41898282+github-actions[bot]@users.noreply.github.com");git("config","user.name","github-actions[bot]");git("add","data/college-content-master.csv","data/college-content-master.xlsx",f"quality-audit-{slugify(college)}.json",*[fn for fn,_ in safe]);git("commit","-m",f"Hold {college} pages for QA revision");git("push","origin","main")
